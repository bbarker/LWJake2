...
public class Netchan : SV_MAIN() {
     ...
     */
        ...
        public var showpackets: cvar_t

        public var showdrop: cvar_t

        public var qport: cvar_t

        //public static netadr_t net_from = new netadr_t();
        public var net_message: sizebuf_t = sizebuf_t()

        public var net_message_buffer: ByteArray = ByteArray(Defines.MAX_MSGLEN)
     ...
     */
        //ok.
        public fun Netchan_Init() {
            val port: Long

            // pick a port value that should be nice and random
            port = Timer.Milliseconds() and 65535
        ...
        private val send_buf = ByteArray(Defines.MAX_MSGLEN)
        private val send = sizebuf_t()
        /*
     * =============== Netchan_OutOfBand
     * 
     ...
     */
        //ok.
        public fun Netchan_OutOfBand(net_socket: Int, adr: netadr_t, length: Int, data: ByteArray) {
        ...
        public fun OutOfBandPrint(net_socket: Int, adr: netadr_t, s: String) {
     ...
     */
        public fun Setup(sock: Int, chan: netchan_t, adr: netadr_t, qport: Int) {
     ...
     */
        public fun Netchan_CanReliable(chan: netchan_t): Boolean {
        ...
        // das ist richtig !!!
        public fun Netchan_NeedReliable(chan: netchan_t): Boolean {
            var send_reliable: Boolean
     ...
     * messages. ================
     */
        public fun Transmit(chan: netchan_t, length: Int, data: ByteArray) {
            val send_reliable: Int
            val w1: Int
            val w2: Int
            ...
            send_reliable = if (Netchan_NeedReliable(chan)) 1 else 0
            ...
            // write the packet header
            SZ.Init(send, send_buf, send_buf.size())

            w1 = (chan.outgoing_sequence and (1 shl 31).inv()) or (send_reliable shl 31)
            w2 = (chan.incoming_sequence and (1 shl 31).inv()) or (chan.incoming_reliable_sequence shl 31)

            chan.outgoing_sequence++
            chan.last_sent = Globals.curtime as Int
            ...
            if (chan.sock == Defines.NS_CLIENT)
                MSG.WriteShort(send, qport.value as Int)
     ...
     */
        public fun Process(chan: netchan_t, msg: sizebuf_t): Boolean {
            var sequence: Int
            var sequence_ack: Int
            val reliable_ack: Int
            val reliable_message: Int
            ...
            reliable_message = sequence.ushr(31)
            reliable_ack = sequence_ack.ushr(31)

            sequence = sequence and (1 shl 31).inv()
            sequence_ack = sequence_ack and (1 shl 31).inv()

            if (showpackets.value != 0) {
                if (reliable_message != 0)
                    Com.Printf(//"recv %4i : s=%i reliable=%i ack=%i rack=%i\n"
                            "recv " + msg.cursize + " : s=" + sequence + " reliable=" + (chan.incoming_reliable_sequence xor 1) + " ack=" + sequence_ack + " rack=" + reliable_ack + "\n")
                else
            ...
            //
            chan.last_received = Globals.curtime as Int
