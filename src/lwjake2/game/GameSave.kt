...
public class GameSave {
    companion object {
        ...
        public fun InitGame() {
            ...
            for (n in preloadclasslist.indices) {
                try {
            ...
            // latched vars
            GameBase.sv_cheats = GameBase.gi.cvar("cheats", "0", Defines.CVAR_SERVERINFO or Defines.CVAR_LATCH)
            GameBase.gi.cvar("gamename", Defines.GAMEVERSION, Defines.CVAR_SERVERINFO or Defines.CVAR_LATCH)
            GameBase.gi.cvar("gamedate", Globals.__DATE__, Defines.CVAR_SERVERINFO or Defines.CVAR_LATCH)

            GameBase.maxclients = GameBase.gi.cvar("maxclients", "4", Defines.CVAR_SERVERINFO or Defines.CVAR_LATCH)
            ...
            GameBase.game.maxentities = GameBase.maxentities.value as Int
            CreateEdicts()

            // initialize all clients for this game
            GameBase.game.maxclients = GameBase.maxclients.value as Int
         ...
         * position.
         */
        public fun WriteGame(filename: String, autosave: Boolean) {
            try {
                val f: QuakeFile
        ...
        public fun ReadGame(filename: String) {

            var f: QuakeFile? = null
         ...
         * WriteLevel
         */
        public fun WriteLevel(filename: String) {
            try {
                var i: Int
                var ent: edict_t
                val f: QuakeFile
         ...
         */
        public fun ReadLevel(filename: String) {
            try {
                val ent: edict_t

                val f = QuakeFile(filename, "r")

                // wipe all the entities
                CreateEdicts()

                GameBase.num_edicts = GameBase.maxclients.value as Int + 1
                ...
                for (i in 0..GameBase.maxclients.value - 1) {
                ...
                for (i in 0..GameBase.num_edicts - 1) {
                    ent = GameBase.g_edicts[i]
