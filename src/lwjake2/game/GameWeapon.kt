...
public class GameWeapon {
    companion object {
        ...
        var blaster_touch: EntTouchAdapter = object : EntTouchAdapter() {
            ...
            public fun touch(self: edict_t, other: edict_t, plane: cplane_t?, surf: csurface_t?) {
                ...
                if (surf != null && (surf!!.flags and Defines.SURF_SKY) != 0) {
        ...
        var Grenade_Explode: EntThinkAdapter = object : EntThinkAdapter() {
            public fun getID(): String {
            ...
            public fun think(ent: edict_t): Boolean {
                ...
                if (ent.enemy != null) {
                    ...
                    Math3D.VectorMA(ent.enemy.s.origin, 0.5.toFloat(), v, v)
                    Math3D.VectorSubtract(ent.s.origin, v, v)
                    points = ent.dmg - 0.5.toFloat() * Math3D.VectorLength(v)
                    ...
                    GameCombat.T_Damage(ent.enemy, ent, ent.owner, dir, ent.s.origin, Globals.vec3_origin, points.toInt(), points.toInt(), Defines.DAMAGE_RADIUS, mod)
                }

                if ((ent.spawnflags and 2) != 0)
                    mod = Defines.MOD_HELD_GRENADE
                else if ((ent.spawnflags and 1) != 0)
                ...
                Math3D.VectorMA(ent.s.origin, -0.02.toFloat(), ent.velocity, origin)
                GameBase.gi.WriteByte(Defines.svc_temp_entity)
                if (ent.waterlevel != 0) {
        ...
        var Grenade_Touch: EntTouchAdapter = object : EntTouchAdapter() {
            public fun getID(): String {
            ...
            public fun touch(ent: edict_t, other: edict_t, plane: cplane_t, surf: csurface_t?) {
                ...
                if (surf != null && 0 != (surf!!.flags and Defines.SURF_SKY)) {
                    GameUtil.G_FreeEdict(ent)
                    return
                }
     ...
     * =================
     */
        var rocket_touch: EntTouchAdapter = object : EntTouchAdapter() {
            public fun getID(): String {
                return "rocket_touch"
            }
            ...
            public fun touch(ent: edict_t, other: edict_t, plane: cplane_t, surf: csurface_t?) {
                ...
                if (other.takedamage != 0) {
                    ...
                    if (GameBase.deathmatch.value == 0 && 0 == GameBase.coop.value) {
                        if ((surf != null) && 0 == (surf!!.flags and (Defines.SURF_WARP or Defines.SURF_TRANS33 or Defines.SURF_TRANS66 or Defines.SURF_FLOWING))) {
                            n = Lib.rand() % 5
                            while (n-- > 0)
     ...
     * =================
     */
        var bfg_explode: EntThinkAdapter = object : EntThinkAdapter() {
            public fun getID(): String {
                return "bfg_explode"
            }
            ...
            public fun think(self: edict_t): Boolean {
                ...
                if (self.s.frame == 0) {
                    ...
                    while ((edit = GameBase.findradius(edit, self.s.origin, self.dmg_radius)) != null) {
                        ...
                        points = (self.radius_dmg * (1.0 - Math.sqrt(dist / self.dmg_radius))) as Float
                        if (ent == self.owner)
                        ...
                        GameBase.gi.multicast(ent!!.s.origin, Defines.MULTICAST_PHS)
                        GameCombat.T_Damage(ent, self, self.owner, self.velocity, ent!!.s.origin, Globals.vec3_origin, points.toInt(), 0, Defines.DAMAGE_ENERGY, Defines.MOD_BFG_EFFECT)
                    }
                }
        ...
        var bfg_touch: EntTouchAdapter = object : EntTouchAdapter() {
            public fun getID(): String {
                return "bfg_touch"
            }
            ...
            public fun touch(self: edict_t, other: edict_t, plane: cplane_t, surf: csurface_t?) {
                ...
                self.s.frame = 0
                self.s.sound = 0
                self.s.effects = self.s.effects and Defines.EF_ANIM_ALLFAST.inv()
        ...
        var bfg_think: EntThinkAdapter = object : EntThinkAdapter() {
            public fun getID(): String {
                return "bfg_think"
            }
            ...
            public fun think(self: edict_t): Boolean {
                ...
                while ((edit = GameBase.findradius(edit, self.s.origin, 256)) != null) {
                    ...
                    if (0 == (ent.svflags and Defines.SVF_MONSTER) && (null == ent.client) && (Lib.strcmp(ent.classname, "misc_explobox") != 0))
                        continue

                    Math3D.VectorMA(ent.absmin, 0.5.toFloat(), ent.size, point)
                    ...
                    while (true) {
                        tr = GameBase.gi.trace(start, null, null, end, ignore, Defines.CONTENTS_SOLID or Defines.CONTENTS_MONSTER or Defines.CONTENTS_DEADMONSTER)

                        if (null == tr.ent)
                            break

                        // hurt it if we can
                        if ((tr.ent.takedamage != 0) && 0 == (tr.ent.flags and Defines.FL_IMMUNE_LASER) && (tr.ent != self.owner))
                        ...
                        // done
                        if (0 == (tr.ent.svflags and Defines.SVF_MONSTER) && (null == tr.ent.client)) {
                            GameBase.gi.WriteByte(Defines.svc_temp_entity)
     ...
     * =================
     */
        fun check_dodge(self: edict_t, start: FloatArray, dir: FloatArray, speed: Int) {
            val end = floatArray(0.0, 0.0, 0.0)
            val v = floatArray(0.0, 0.0, 0.0)
            val tr: trace_t
            val eta: Float
            ...
            if ((tr.ent != null) && (tr.ent.svflags and Defines.SVF_MONSTER) != 0 && (tr.ent.health > 0) && (null != tr.ent.monsterinfo.dodge) && GameUtil.infront(tr.ent, self)) {
     ...
     * =================
     */
        public fun fire_hit(self: edict_t, aim: FloatArray, damage: Int, kick: Int): Boolean {
            val tr: trace_t
            ...
            if (tr.fraction < 1) {
                ...
                // hit
                if ((tr.ent.svflags and Defines.SVF_MONSTER) != 0 || (tr.ent.client != null))
                    tr.ent = self.enemy
            }
            ...
            Math3D.VectorMA(self.enemy.absmin, 0.5.toFloat(), self.enemy.size, v)
     ...
     * =================
     */
        public fun fire_lead(self: edict_t, start: FloatArray, aimdir: FloatArray, damage: Int, kick: Int, te_impact: Int, hspread: Int, vspread: Int, mod: Int) {
            var tr: trace_t
            val dir = floatArray(0.0, 0.0, 0.0)
            val forward = floatArray(0.0, 0.0, 0.0)
            val right = floatArray(0.0, 0.0, 0.0)
            ...
            var r: Float
            var u: Float
            val water_start = floatArray(0.0, 0.0, 0.0)
            var water = false
            var content_mask = Defines.MASK_SHOT or Defines.MASK_WATER
            ...
            if (!(tr.fraction < 1.0)) {
                ...
                if ((GameBase.gi.pointcontents.pointcontents(start) and Defines.MASK_WATER) != 0) {
                    water = true
                    Math3D.VectorCopy(start, water_start)
                    content_mask = content_mask and Defines.MASK_WATER.inv()
                }

                tr = GameBase.gi.trace(start, null, null, end, self, content_mask)

                // see if we hit water
                if ((tr.contents and Defines.MASK_WATER) != 0) {
                    val color: Int
                    ...
                    if (!Math3D.VectorEquals(start, tr.endpos)) {
                        if ((tr.contents and Defines.CONTENTS_WATER) != 0) {
                            if (Lib.strcmp(tr.surface.name, "*brwater") == 0)
                                color = Defines.SPLASH_BROWN_WATER
                            else
                                color = Defines.SPLASH_BLUE_WATER
                        } else if ((tr.contents and Defines.CONTENTS_SLIME) != 0)
                            color = Defines.SPLASH_SLIME
                        else if ((tr.contents and Defines.CONTENTS_LAVA) != 0)
            ...
            // send gun puff / flash
            if (!((tr.surface != null) && 0 != (tr.surface.flags and Defines.SURF_SKY))) {
                if (tr.fraction < 1.0) {
            ...
            // trail
            if (water) {
                val pos = floatArray(0.0, 0.0, 0.0)

                Math3D.VectorSubtract(tr.endpos, water_start, dir)
                Math3D.VectorNormalize(dir)
                Math3D.VectorMA(tr.endpos, -2, dir, pos)
                if ((GameBase.gi.pointcontents.pointcontents(pos) and Defines.MASK_WATER) != 0)
                ...
                Math3D.VectorScale(pos, 0.5.toFloat(), pos)
     ...
     * pistols, rifles, etc.... =================
     */
        public fun fire_bullet(self: edict_t, start: FloatArray, aimdir: FloatArray, damage: Int, kick: Int, hspread: Int, vspread: Int, mod: Int) {
     ...
     * =================
     */
        public fun fire_shotgun(self: edict_t, start: FloatArray, aimdir: FloatArray, damage: Int, kick: Int, hspread: Int, vspread: Int, count: Int, mod: Int) {
            var i: Int
     ...
     */
        ...
        public fun fire_blaster(self: edict_t, start: FloatArray, dir: FloatArray, damage: Int, speed: Int, effect: Int, hyper: Boolean) {
            val bolt: edict_t
            val tr: trace_t
        ...
        public fun fire_grenade(self: edict_t, start: FloatArray, aimdir: FloatArray, damage: Int, speed: Int, timer: Float, damage_radius: Float) {
            val grenade: edict_t
            val dir = floatArray(0.0, 0.0, 0.0)
            val forward = floatArray(0.0, 0.0, 0.0)
            val right = floatArray(0.0, 0.0, 0.0)
            ...
            Math3D.VectorMA(grenade.velocity, 200.toFloat() + Lib.crandom() * 10.0.toFloat(), up, grenade.velocity)
            Math3D.VectorMA(grenade.velocity, Lib.crandom() * 10.0.toFloat(), right, grenade.velocity)
            ...
            grenade.s.effects = grenade.s.effects or Defines.EF_GRENADE
        ...
        public fun fire_grenade2(self: edict_t, start: FloatArray, aimdir: FloatArray, damage: Int, speed: Int, timer: Float, damage_radius: Float, held: Boolean) {
            val grenade: edict_t
            val dir = floatArray(0.0, 0.0, 0.0)
            val forward = floatArray(0.0, 0.0, 0.0)
            val right = floatArray(0.0, 0.0, 0.0)
            ...
            Math3D.VectorMA(grenade.velocity, 200.toFloat() + Lib.crandom() * 10.0.toFloat(), up, grenade.velocity)
            Math3D.VectorMA(grenade.velocity, Lib.crandom() * 10.0.toFloat(), right, grenade.velocity)
            Math3D.VectorSet(grenade.avelocity, 300.toFloat(), 300.toFloat(), 300.toFloat())
            grenade.movetype = Defines.MOVETYPE_BOUNCE
            grenade.clipmask = Defines.MASK_SHOT
            grenade.solid = Defines.SOLID_BBOX
            grenade.s.effects = grenade.s.effects or Defines.EF_GRENADE
        ...
        public fun fire_rocket(self: edict_t, start: FloatArray, dir: FloatArray, damage: Int, speed: Int, damage_radius: Float, radius_damage: Int) {
            val rocket: edict_t
            ...
            rocket.s.effects = rocket.s.effects or Defines.EF_ROCKET
     ...
     * =================
     */
        public fun fire_rail(self: edict_t, start: FloatArray, aimdir: FloatArray, damage: Int, kick: Int) {
            val from = floatArray(0.0, 0.0, 0.0)
            val end = floatArray(0.0, 0.0, 0.0)
            var tr: trace_t? = null
            var ignore: edict_t?
            var mask: Int
            ...
            Math3D.VectorMA(start, 8192.toFloat(), aimdir, end)
            Math3D.VectorCopy(start, from)
            ignore = self
            water = false
            mask = Defines.MASK_SHOT or Defines.CONTENTS_SLIME or Defines.CONTENTS_LAVA
            while (ignore != null) {
                tr = GameBase.gi.trace(from, null, null, end, ignore, mask)

                if ((tr!!.contents and (Defines.CONTENTS_SLIME or Defines.CONTENTS_LAVA)) != 0) {
                    mask = mask and (Defines.CONTENTS_SLIME or Defines.CONTENTS_LAVA).inv()
                    water = true
                } else {
                    //ZOID--added so rail goes through SOLID_BBOX entities (gibs,
                    // etc)
                    if ((tr!!.ent.svflags and Defines.SVF_MONSTER) != 0 || (tr!!.ent.client != null) || (tr!!.ent.solid == Defines.SOLID_BBOX))
                        ignore = tr!!.ent
                    else
                        ignore = null
        ...
        public fun fire_bfg(self: edict_t, start: FloatArray, dir: FloatArray, damage: Int, speed: Int, damage_radius: Float) {
            val bfg: edict_t
            ...
            bfg.s.effects = bfg.s.effects or (Defines.EF_BFG or Defines.EF_ANIM_ALLFAST)
