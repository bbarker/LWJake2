...
public class CL_view {
    companion object {
        ...
        var num_cl_weaponmodels: Int = 0

        var cl_weaponmodels = arrayOfNulls<String>(Defines.MAX_CLIENTWEAPONMODELS)
     ...
     */
        fun PrepRefresh() {
            ...
            mapname = Globals.cl.configstrings[Defines.CS_MODELS + 1].substring(5, i - 4) // skip "maps/"
            // cut off ".bsp"
            ...
            run {
                i = 1
                while (i < Defines.MAX_MODELS && Globals.cl.configstrings[Defines.CS_MODELS + i].length() != 0) {
                    name = String(Globals.cl.configstrings[Defines.CS_MODELS + i])
                    if (name.length() > 37)
            ...
            run {
                i = 1
                while (i < Defines.MAX_IMAGES && Globals.cl.configstrings[Defines.CS_IMAGES + i].length() > 0) {
            ...
            Com.Printf("sky\r")
            SCR.UpdateScreen()
            rotate = Float.parseFloat(Globals.cl.configstrings[Defines.CS_SKYROTATE])
            val st = StringTokenizer(Globals.cl.configstrings[Defines.CS_SKYAXIS])
        ...
        public fun AddNetgraph() {
            ...
            // add the net lines
            if (SCR.scr_debuggraph.value == 0.0.toFloat() || SCR.scr_timegraph.value == 0.0.toFloat())
                return

            run {
                i = 0
                while (i < Globals.cls.netchan.dropped) {
                    SCR.DebugGraph(30, 64)
                    i++
                }
            }
            ...
            run {
                i = 0
                while (i < Globals.cl.surpressCount) {
                    SCR.DebugGraph(30, 223)
                    i++
                }
            }

            // see what the latency was on this packet
            `in` = Globals.cls.netchan.incoming_acknowledged and (Defines.CMD_BACKUP - 1)
            ping = (Globals.cls.realtime - Globals.cl.cmd_time[`in`]) as Int
            ping /= 30
            if (ping > 30)
                ping = 30
            SCR.DebugGraph(ping, 208)
        }
    }
}