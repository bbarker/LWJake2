...
public class M {
    companion object {
        ...
        public fun M_CheckGround(ent: edict_t) {
            ...
            point[2] = ent.s.origin[2] - 0.25.toFloat()
                    ...
                    run {
                        ...
                        while (y <= 1) {
                            ...
                            if (GameBase.gi.pointcontents.pointcontents(start) != Defines.CONTENTS_SOLID) {
                                ...
                                start[0] = stop[0] = (mins[0] + maxs[0]) * 0.5.toFloat()
                                start[1] = stop[1] = (mins[1] + maxs[1]) * 0.5.toFloat()
                                ...
                                run {
                                    ...
                                    while (x <= 1) {
                                        run {
                                            y = 0
                                            while (y <= 1) {
                                                start[0] = stop[0] = if (x != 0) maxs[0] else mins[0]
                                                start[1] = stop[1] = if (y != 0) maxs[1] else mins[1]
         ...
         * M_ChangeYaw.
         */
        public fun M_ChangeYaw(ent: edict_t) {
            val ideal: Float
            val current: Float
            var move: Float
            val speed: Float
         ...
         * M_MoveToGoal.
         */
        public fun M_MoveToGoal(ent: edict_t, dist: Float) {
            val goal = ent.goalentity

            if (ent.groundentity == null && (ent.flags and (Defines.FL_FLY or Defines.FL_SWIM)) == 0)
                return
            ...
            //	   bump around...
            if ((Lib.rand() and 3) == 1 || !SV.SV_StepDirection(ent, ent.ideal_yaw, dist)) {
                if (ent.inuse)
         ...
         * M_walkmove.
         */
        public fun M_walkmove(ent: edict_t, yaw: Float, dist: Float): Boolean {
            var yaw = yaw
        ...
        public fun M_CatagorizePosition(ent: edict_t) {
            val point = floatArray(0.0, 0.0, 0.0)
            var cont: Int
            ...
            if (0 == (cont and Defines.MASK_WATER)) {
                ent.waterlevel = 0
                ent.watertype = 0
                return
            }
        ...
        public fun M_WorldEffects(ent: edict_t) {
            var dmg: Int

            if (ent.health > 0) {
                if (0 == (ent.flags and Defines.FL_SWIM)) {
                    if (ent.waterlevel < 3) {
                        ...
                        if (ent.pain_debounce_time < GameBase.level.time) {
                            dmg = (2.toFloat() + 2.toFloat() * Math.floor(GameBase.level.time - ent.air_finished)) as Int
                            if (dmg > 15)
                                dmg = 15
                    ...
                    if (ent.waterlevel > 0) {
                        ...
                        // suffocate!
                        if (ent.pain_debounce_time < GameBase.level.time) {
                            dmg = (2 + 2 * Math.floor(GameBase.level.time - ent.air_finished)) as Int
                            if (dmg > 15)
                                dmg = 15
                            GameCombat.T_Damage(ent, GameBase.g_edicts[0], GameBase.g_edicts[0], Globals.vec3_origin, ent.s.origin, Globals.vec3_origin, dmg, 0, Defines.DAMAGE_NO_ARMOR, Defines.MOD_WATER)
                            ent.pain_debounce_time = GameBase.level.time + 1
            ...
            if (ent.waterlevel == 0) {
                if ((ent.flags and Defines.FL_INWATER) != 0) {
                    GameBase.gi.sound(ent, Defines.CHAN_BODY, GameBase.gi.soundindex("player/watr_out.wav"), 1, Defines.ATTN_NORM, 0)
                    ent.flags = ent.flags and Defines.FL_INWATER.inv()
                }
                return
            }

            if ((ent.watertype and Defines.CONTENTS_LAVA) != 0 && 0 == (ent.flags and Defines.FL_IMMUNE_LAVA)) {
                if (ent.damage_debounce_time < GameBase.level.time) {
                    ent.damage_debounce_time = GameBase.level.time + 0.2.toFloat()
            ...
            if ((ent.watertype and Defines.CONTENTS_SLIME) != 0 && 0 == (ent.flags and Defines.FL_IMMUNE_SLIME)) {
                if (ent.damage_debounce_time < GameBase.level.time) {
            ...
            if (0 == (ent.flags and Defines.FL_INWATER)) {
                if (0 == (ent.svflags and Defines.SVF_DEADMONSTER)) {
                    if ((ent.watertype and Defines.CONTENTS_LAVA) != 0)
                    ...
                    else if ((ent.watertype and Defines.CONTENTS_SLIME) != 0)
                        GameBase.gi.sound(ent, Defines.CHAN_BODY, GameBase.gi.soundindex("player/watr_in.wav"), 1, Defines.ATTN_NORM, 0)
                    else if ((ent.watertype and Defines.CONTENTS_WATER) != 0)
                        GameBase.gi.sound(ent, Defines.CHAN_BODY, GameBase.gi.soundindex("player/watr_in.wav"), 1, Defines.ATTN_NORM, 0)
                }

                ent.flags = ent.flags or Defines.FL_INWATER
                ent.damage_debounce_time = 0
            }
        }

        public var M_droptofloor: EntThinkAdapter = object : EntThinkAdapter() {
        ...
        public fun M_SetEffects(ent: edict_t) {
            ent.s.effects = ent.s.effects and (Defines.EF_COLOR_SHELL or Defines.EF_POWERSCREEN).inv()
            ent.s.renderfx = ent.s.renderfx and (Defines.RF_SHELL_RED or Defines.RF_SHELL_GREEN or Defines.RF_SHELL_BLUE).inv()

            if ((ent.monsterinfo.aiflags and Defines.AI_RESURRECTING) != 0) {
                ent.s.effects = ent.s.effects or Defines.EF_COLOR_SHELL
                ent.s.renderfx = ent.s.renderfx or Defines.RF_SHELL_RED
            }
        ...
        //ok
        public fun M_MoveFrame(self: edict_t) {
            var move: mmove_t //ptr
            val index: Int
            ...
            if ((self.monsterinfo.nextframe != 0) && (self.monsterinfo.nextframe >= move.firstframe) && (self.monsterinfo.nextframe <= move.lastframe)) {
                ...
                if (self.s.frame == move.lastframe) {
                    if (move.endfunc != null) {
                        ...
                        // check for death
                        if ((self.svflags and Defines.SVF_DEADMONSTER) != 0)
                            return
                    }
                }
        ...
        /** Stops the Flies.  */
        public var M_FliesOff: EntThinkAdapter = object : EntThinkAdapter() {
            public fun getID(): String {
                return "m_fliesoff"
            }
        ...
        public var M_FliesOn: EntThinkAdapter = object : EntThinkAdapter() {
            public fun getID(): String {
                return "m_flies_on"
            }
            ...
            public fun think(self: edict_t): Boolean {
                ...
                self.s.effects = self.s.effects or Defines.EF_FLIES
        ...
        public var M_FlyCheck: EntThinkAdapter = object : EntThinkAdapter() {
            public fun getID(): String {
                return "m_fly_check"
            }
