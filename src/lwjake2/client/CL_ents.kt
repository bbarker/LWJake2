...
public class CL_ents {
    companion object {
        ...
        var bitcounts = IntArray(32) /// just for protocol profiling

        var bfg_lightramp = intArray(300, 400, 600, 300, 150, 75)
	 ...
	 */
        public fun ParseEntityBits(bits: IntArray): Int {
            val b: Int
            var total: Int
            var i: Int
            val number: Int
            ...
            if ((total and Defines.U_MOREBITS1) != 0) {

                b = MSG.ReadByte(Globals.net_message)
                total = total or (b shl 8)
            }
            if ((total and Defines.U_MOREBITS2) != 0) {

                b = MSG.ReadByte(Globals.net_message)
                total = total or (b shl 16)
            }
            if ((total and Defines.U_MOREBITS3) != 0) {
            ...
            run {
                i = 0
                while (i < 32) {
                    if ((total and (1 shl i)) != 0)
                        bitcounts[i]++
                    i++
                }
            }

            if ((total and Defines.U_NUMBER16) != 0)
	 ...
	 * ==================
	 */
        public fun ParseDelta(from: entity_state_t, to: entity_state_t, number: Int, bits: Int) {
	 ...
	 * current frame ==================
	 */
        public fun DeltaEntity(frame: frame_t, newnum: Int, old: entity_state_t, bits: Int) {
            val ent: centity_t
            val state: entity_state_t

            ent = Globals.cl_entities[newnum]

            state = Globals.cl_parse_entities[Globals.cl.parse_entities and (Defines.MAX_PARSE_ENTITIES - 1)]
	 ...
	 * data stream. ==================
	 */
        public fun ParsePacketEntities(oldframe: frame_t?, newframe: frame_t) {
            val newnum: Int
            var bits = 0
            ...
            if (oldframe == null)
                ...
                //				oldnum = 99999;
                //			else {
                oldstate = Globals.cl_parse_entities[(oldframe!!.parse_entities + oldindex) and (Defines.MAX_PARSE_ENTITIES - 1)]
            ...
            while (true) {
                ...
                if ((bits and Defines.U_REMOVE) != 0) {
	 ...
	 * =================== CL_ParsePlayerstate ===================
	 */
        public fun ParsePlayerstate(oldframe: frame_t?, newframe: frame_t) {
            val flags: Int
            val state: player_state_t
            var i: Int
            val statbits: Int
            ...
            //
            if ((flags and Defines.PS_M_TYPE) != 0)
                state.pmove.pm_type = MSG.ReadByte(Globals.net_message)

            if ((flags and Defines.PS_M_ORIGIN) != 0) {
            ...
            if ((flags and Defines.PS_M_VELOCITY) != 0) {
            ...
            if ((flags and Defines.PS_M_TIME) != 0) {
                state.pmove.pm_time = MSG.ReadByte(Globals.net_message) as Byte
            }

            if ((flags and Defines.PS_M_FLAGS) != 0)
                state.pmove.pm_flags = MSG.ReadByte(Globals.net_message) as Byte

            if ((flags and Defines.PS_M_GRAVITY) != 0)
                state.pmove.gravity = MSG.ReadShort(Globals.net_message)

            if ((flags and Defines.PS_M_DELTA_ANGLES) != 0) {
            ...
            //
            if ((flags and Defines.PS_VIEWOFFSET) != 0) {
                state.viewoffset[0] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.viewoffset[1] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.viewoffset[2] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
            }

            if ((flags and Defines.PS_VIEWANGLES) != 0) {
            ...
            if ((flags and Defines.PS_KICKANGLES) != 0) {

                state.kick_angles[0] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.kick_angles[1] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.kick_angles[2] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()

            }

            if ((flags and Defines.PS_WEAPONINDEX) != 0) {
                state.gunindex = MSG.ReadByte(Globals.net_message)
            }

            if ((flags and Defines.PS_WEAPONFRAME) != 0) {
                state.gunframe = MSG.ReadByte(Globals.net_message)
                state.gunoffset[0] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.gunoffset[1] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.gunoffset[2] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.gunangles[0] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.gunangles[1] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
                state.gunangles[2] = MSG.ReadChar(Globals.net_message) * 0.25.toFloat()
            }

            if ((flags and Defines.PS_BLEND) != 0) {
                state.blend[0] = MSG.ReadByte(Globals.net_message) / 255.0.toFloat()
                state.blend[1] = MSG.ReadByte(Globals.net_message) / 255.0.toFloat()
                state.blend[2] = MSG.ReadByte(Globals.net_message) / 255.0.toFloat()
                state.blend[3] = MSG.ReadByte(Globals.net_message) / 255.0.toFloat()
            }

            if ((flags and Defines.PS_FOV) != 0)
                state.fov = MSG.ReadByte(Globals.net_message)

            if ((flags and Defines.PS_RDFLAGS) != 0)
	 ...
	 * ==================
	 */
        public fun FireEntityEvents(frame: frame_t) {
            var s1: entity_state_t
            var pnum: Int
            var num: Int
            ...
            run {
                ...
                while (pnum < frame.num_entities) {
                    num = (frame.parse_entities + pnum) and (Defines.MAX_PARSE_ENTITIES - 1)
                    s1 = Globals.cl_parse_entities[num]
                    if (s1.event != 0)
                        CL_fx.EntityEvent(s1)

                    // EF_TELEPORTER acts like an event, but is not cleared each frame
                    if ((s1.effects and Defines.EF_TELEPORTER) != 0)
	 ...
	 */
        public fun ParseFrame() {
            ...
            if (Globals.cl.frame.deltaframe <= 0) {
                ...
                old = Globals.cl.frames[Globals.cl.frame.deltaframe and Defines.UPDATE_MASK]
                if (!old!!.valid) {
                    // should never happen
            ...
            Globals.cl.frames[Globals.cl.frame.serverframe and Defines.UPDATE_MASK].set(Globals.cl.frame)
            ...
            if (Globals.cl.frame.valid) {
                ...
                if (Globals.cls.state != Defines.ca_active) {
                    ...
                    Globals.cl.predicted_origin[0] = Globals.cl.frame.playerstate.pmove.origin[0] * 0.125.toFloat()
                    Globals.cl.predicted_origin[1] = Globals.cl.frame.playerstate.pmove.origin[1] * 0.125.toFloat()
                    Globals.cl.predicted_origin[2] = Globals.cl.frame.playerstate.pmove.origin[2] * 0.125.toFloat()
	 ...
	 * ===============
	 */
        fun AddPacketEntities(frame: frame_t) {
            var s1: entity_state_t
            val autorotate: Float
            var i: Int
            var pnum: Int
            var cent: centity_t
            val autoanim: Int
            ...
            run {
                pnum = 0
                while (pnum < frame.num_entities) {
                    s1 = Globals.cl_parse_entities[(frame.parse_entities + pnum) and (Defines.MAX_PARSE_ENTITIES - 1)]
                    ...
                    // set frame
                    if ((effects and Defines.EF_ANIM01) != 0)
                        ent.frame = autoanim and 1
                    else if ((effects and Defines.EF_ANIM23) != 0)
                        ent.frame = 2 + (autoanim and 1)
                    else if ((effects and Defines.EF_ANIM_ALL) != 0)
                        ent.frame = autoanim
                    else if ((effects and Defines.EF_ANIM_ALLFAST) != 0)
                    ...
                    if ((effects and Defines.EF_PENT) != 0) {
                        effects = effects and Defines.EF_PENT.inv()
                        effects = effects or Defines.EF_COLOR_SHELL
                        renderfx = renderfx or Defines.RF_SHELL_RED
                    }

                    if ((effects and Defines.EF_QUAD) != 0) {
                        effects = effects and Defines.EF_QUAD.inv()
                        effects = effects or Defines.EF_COLOR_SHELL
                        renderfx = renderfx or Defines.RF_SHELL_BLUE
                    }
                    //	  ======
                    //	   PMM
                    if ((effects and Defines.EF_DOUBLE) != 0) {
                        effects = effects and Defines.EF_DOUBLE.inv()
                        effects = effects or Defines.EF_COLOR_SHELL
                        renderfx = renderfx or Defines.RF_SHELL_DOUBLE
                    }

                    if ((effects and Defines.EF_HALF_DAMAGE) != 0) {
                        effects = effects and Defines.EF_HALF_DAMAGE.inv()
                        effects = effects or Defines.EF_COLOR_SHELL
                        renderfx = renderfx or Defines.RF_SHELL_HALF_DAM
                    }
                    //	   pmm
                    //	  ======
                    ...
                    if ((renderfx and Defines.RF_BEAM) != 0) {
                        // the four beam colors are
                        // encoded in 32 bits of
                        // skinnum (hack)
                        ent.alpha = 0.30.toFloat()
                        ent.skinnum = (s1.skinnum shr ((Globals.rnd.nextInt(4)) * 8)) and 255
                        Math.random()
                        ent.model = null
                    } else {
                        // set skin
                        if (s1.modelindex == 255) {
                            ...
                            ent.skinnum = 0
                            ci = Globals.cl.clientinfo[s1.skinnum and 255]
                            ent.skin = ci.skin
                            ent.model = ci.model
                            ...
                            //	  ============
                            //	  PGM
                            if ((renderfx and Defines.RF_USE_DISGUISE) != 0) {
                    ...
                    if (renderfx == Defines.RF_TRANSLUCENT)
                        ent.alpha = 0.70.toFloat()

                    // render effects (fullbright, translucent, etc)
                    if ((effects and Defines.EF_COLOR_SHELL) != 0)
                        ent.flags = 0 // renderfx go on color shell entity
                    else
                        ent.flags = renderfx

                    // calculate angles
                    if ((effects and Defines.EF_ROTATE) != 0) {
                        // some bonus items
                        // auto-rotate
                        ent.angles[0] = 0
                        ent.angles[1] = autorotate
                        ent.angles[2] = 0
                    } else if ((effects and Defines.EF_SPINNINGLIGHTS) != 0) {
                        ent.angles[0] = 0
                    ...
                    if (s1.number == Globals.cl.playernum + 1) {
                        ent.flags = ent.flags or Defines.RF_VIEWERMODEL // only draw from mirrors
                        // FIXME: still pass to refresh

                        if ((effects and Defines.EF_FLAG1) != 0)
                            V.AddLight(ent.origin, 225, 1.0.toFloat(), 0.1.toFloat(), 0.1.toFloat())
                        else if ((effects and Defines.EF_FLAG2) != 0)
                            V.AddLight(ent.origin, 225, 0.1.toFloat(), 0.1.toFloat(), 1.0.toFloat())
                        else if ((effects and Defines.EF_TAGTRAIL) != 0)
                        //PGM
                            V.AddLight(ent.origin, 225, 1.0.toFloat(), 1.0.toFloat(), 0.0.toFloat()) //PGM
                        else if ((effects and Defines.EF_TRACKERTRAIL) != 0)
                        //PGM
                            V.AddLight(ent.origin, 225, -1.0.toFloat(), -1.0.toFloat(), -1.0.toFloat()) //PGM
                    ...
                    if ((effects and Defines.EF_PLASMA) != 0) {
                        ent.flags = ent.flags or Defines.RF_TRANSLUCENT
                        ent.alpha = 0.6.toFloat()
                    }

                    if ((effects and Defines.EF_SPHERETRANS) != 0) {
                        ent.flags = ent.flags or Defines.RF_TRANSLUCENT
                        // PMM - *sigh* yet more EF overloading
                        if ((effects and Defines.EF_TRACKERTRAIL) != 0)
                            ent.alpha = 0.6.toFloat()
                        else
                            ent.alpha = 0.3.toFloat()
                    }
                    //	  pmm
				 ...
				 */
                        if ((renderfx and Defines.RF_SHELL_HALF_DAM) != 0) {
                            if (FS.Developer_searchpath(2) == 2) {
                                ...
                                // double are on
                                if ((renderfx and (Defines.RF_SHELL_RED or Defines.RF_SHELL_BLUE or Defines.RF_SHELL_DOUBLE)) != 0)
                                    renderfx = renderfx and Defines.RF_SHELL_HALF_DAM.inv()
                            }
                        }
                        ...
                        if ((renderfx and Defines.RF_SHELL_DOUBLE) != 0) {
                            if (FS.Developer_searchpath(2) == 2) {
                                ...
                                // green shell
                                if ((renderfx and (Defines.RF_SHELL_RED or Defines.RF_SHELL_BLUE or Defines.RF_SHELL_GREEN)) != 0)
                                    renderfx = renderfx and Defines.RF_SHELL_DOUBLE.inv()
                                // if we have a red shell, turn it to purple by adding
                                // blue
                                if ((renderfx and Defines.RF_SHELL_RED) != 0)
                                    renderfx = renderfx or Defines.RF_SHELL_BLUE
                                else if ((renderfx and Defines.RF_SHELL_BLUE) != 0)
                                // go to green if it's on already, otherwise do cyan
                                // (flash green)
                                    if ((renderfx and Defines.RF_SHELL_GREEN) != 0)
                                        renderfx = renderfx and Defines.RF_SHELL_BLUE.inv()
                                    else
                                        renderfx = renderfx or Defines.RF_SHELL_GREEN// if we have a blue shell (and not a red shell), turn
                                // it to cyan by adding green
                            }
                        }
                        //				}
                        // pmm
                        ent.flags = renderfx or Defines.RF_TRANSLUCENT
                        ent.alpha = 0.30.toFloat()
                        V.AddEntity(ent)
                    }
                    ...
                    if ((effects and Defines.EF_POWERSCREEN) != 0) {
                        ent.model = CL_tent.cl_mod_powerscreen
                        ent.oldframe = 0
                        ent.frame = 0
                        ent.flags = ent.flags or (Defines.RF_TRANSLUCENT or Defines.RF_SHELL_GREEN)
                        ent.alpha = 0.30.toFloat()
                        V.AddEntity(ent)
                    }

                    // add automatic particle trails
                    if ((effects and Defines.EF_ROTATE.inv()) != 0) {
                        if ((effects and Defines.EF_ROCKET) != 0) {
                            CL_fx.RocketTrail(cent.lerp_origin, ent.origin, cent)
                            V.AddLight(ent.origin, 200, 1, 1, 0)
                        } else if ((effects and Defines.EF_BLASTER) != 0) {
                            ...
                            //	  PGM
                        } else if ((effects and Defines.EF_HYPERBLASTER) != 0) {
                            if ((effects and Defines.EF_TRACKER) != 0)
                            // PGM overloaded
                            // for blaster2.
                            ...
                            // PGM
                                V.AddLight(ent.origin, 200, 1, 1, 0)
                        } else if ((effects and Defines.EF_GIB) != 0) {
                            CL_fx.DiminishingTrail(cent.lerp_origin, ent.origin, cent, effects)
                        } else if ((effects and Defines.EF_GRENADE) != 0) {
                            CL_fx.DiminishingTrail(cent.lerp_origin, ent.origin, cent, effects)
                        } else if ((effects and Defines.EF_FLIES) != 0) {
                            CL_fx.FlyEffect(cent, ent.origin)
                        } else if ((effects and Defines.EF_BFG) != 0) {

                            if ((effects and Defines.EF_ANIM_ALLFAST) != 0) {
                        ...
                        } else if ((effects and Defines.EF_TRAP) != 0) {
                            ent.origin[2] += 32
                            CL_fx.TrapParticles(ent)
                            i = (Globals.rnd.nextInt(100)) + 100
                            V.AddLight(ent.origin, i, 1, 0.8.toFloat(), 0.1.toFloat())
                        } else if ((effects and Defines.EF_FLAG1) != 0) {
                            CL_fx.FlagTrail(cent.lerp_origin, ent.origin, 242)
                            V.AddLight(ent.origin, 225, 1, 0.1.toFloat(), 0.1.toFloat())
                        } else if ((effects and Defines.EF_FLAG2) != 0) {
                            CL_fx.FlagTrail(cent.lerp_origin, ent.origin, 115)
                            V.AddLight(ent.origin, 225, 0.1.toFloat(), 0.1.toFloat(), 1)
                        } else if ((effects and Defines.EF_TAGTRAIL) != 0) {
                            CL_newfx.TagTrail(cent.lerp_origin, ent.origin, 220)
                            V.AddLight(ent.origin, 225, 1.0.toFloat(), 1.0.toFloat(), 0.0.toFloat())
                        } else if ((effects and Defines.EF_TRACKERTRAIL) != 0) {
                            if ((effects and Defines.EF_TRACKER) != 0) {
                                val intensity: Float

                                intensity = (50 + (500 * (Math.sin(Globals.cl.time / 500.0) + 1.0))) as Float
                                // FIXME - check out this effect in rendition
                                if (Globals.vidref_val == Defines.VIDREF_GL)
                                    V.AddLight(ent.origin, intensity, -1.0.toFloat(), -1.0.toFloat(), -1.0.toFloat())
                                else
                                    V.AddLight(ent.origin, -1.0.toFloat() * intensity, 1.0.toFloat(), 1.0.toFloat(), 1.0.toFloat())
                            } else {
                                CL_newfx.Tracker_Shell(cent.lerp_origin)
                                V.AddLight(ent.origin, 155, -1.0.toFloat(), -1.0.toFloat(), -1.0.toFloat())
                            }
                        } else if ((effects and Defines.EF_TRACKER) != 0) {
                            ...
                            if (Globals.vidref_val == Defines.VIDREF_GL)
                                ...
                                V.AddLight(ent.origin, -200, 1, 1, 1)
                        } else if ((effects and Defines.EF_GREENGIB) != 0) {
                            CL_fx.DiminishingTrail(cent.lerp_origin, ent.origin, cent, effects)
                        } else if ((effects and Defines.EF_IONRIPPER) != 0) {
                            CL_fx.IonripperTrail(cent.lerp_origin, ent.origin)
                            V.AddLight(ent.origin, 100, 1, 0.5.toFloat(), 0.5.toFloat())
                        } else if ((effects and Defines.EF_BLUEHYPERBLASTER) != 0) {
                            V.AddLight(ent.origin, 200, 0, 0, 1)
                        } else if ((effects and Defines.EF_PLASMA) != 0) {
                            if ((effects and Defines.EF_ANIM_ALLFAST) != 0) {
                                CL_fx.BlasterTrail(cent.lerp_origin, ent.origin)
                            }
                            V.AddLight(ent.origin, 130, 1, 0.5.toFloat(), 0.5.toFloat())
                        }// RAFAEL
                        // RAFAEL
                        // RAFAEL
                        //	  ROGUE
        ...
        // stack variable
        private val gun = entity_t()

        /*
	 * ============== CL_AddViewWeapon ==============
	 */
        fun AddViewWeapon(ps: player_state_t, ops: player_state_t) {
            var i: Int
            ...
            gun.flags = Defines.RF_MINLIGHT or Defines.RF_DEPTHHACK or Defines.RF_WEAPONMODEL
            gun.backlerp = 1.0.toFloat() - Globals.cl.lerpfrac
	 ...
	 */
        fun CalcViewValues() {
            ...
            var ops: player_state_t

            // find the previous frame to interpolate from
            ps = Globals.cl.frame.playerstate

            i = (Globals.cl.frame.serverframe - 1) and Defines.UPDATE_MASK
            ...
            if ((Globals.cl_predict.value != 0) && 0 == (Globals.cl.frame.playerstate.pmove.pm_flags and pmove_t.PMF_NO_PREDICTION)) {
                // use
                // predicted
                // values
                val delta: Int

                backlerp = 1.0.toFloat() - lerp
                run {
                    i = 0
                    while (i < 3) {
                ...
                delta = (Globals.cls.realtime - Globals.cl.predicted_step_time) as Int
                if (delta < 100)
                ...
                run {
                    i = 0
                    while (i < 3) {
                        Globals.cl.refdef.vieworg[i] = ops.pmove.origin[i] * 0.125.toFloat() + ops.viewoffset[i] + lerp * (ps.pmove.origin[i] * 0.125.toFloat() + ps.viewoffset[i] - (ops.pmove.origin[i] * 0.125.toFloat() + ops.viewoffset[i]))
                        i++
                    }
                }
            }
	 ...
	 */
        fun AddEntities() {
            ...
            if (Globals.cl.time > Globals.cl.frame.servertime) {
                ...
                Globals.cl.time = Globals.cl.frame.servertime
                Globals.cl.lerpfrac = 1.0.toFloat()
            } else if (Globals.cl.time < Globals.cl.frame.servertime - 100) {
                if (Globals.cl_showclamp.value != 0)
                ...
                Globals.cl.lerpfrac = 1.0.toFloat() - (Globals.cl.frame.servertime - Globals.cl.time) * 0.01.toFloat()
	 ...
	 */
        public fun GetEntitySoundOrigin(ent: Int, org: FloatArray) {
            val old: centity_t
